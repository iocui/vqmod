...
abstract class VQMod {
	 ...
	 * VQMod::bootup()
	 *
	 * @param string $path File path to use Edit By PGcao
	 * @param bool $logging Enable/disabled logging
	 * @return null
	 * @description Startup of VQMod
	 */
	public static function bootup($path = '', $logging = true) { //Edit By PGcao
	...
	public static function modCheck($sourceFile, $modificationFile = false) {
		...
		foreach(self::$_mods as $modObject) {
			foreach($modObject->mods as $path => $mods) {
				### 去掉要修改的PHP文档绝对路径后补回 Add By PGCAO
				$path = VQMod::getCwd() . $path;
	 ...
	 * @return bool|string Edit By PGcao
	...
	private static function _getMods() {
		...
		// self::$_modFileList = glob(self::path('vqmod/xml/', true) . '*.xml'); // 原方式文件 Remove By PGcao

		// 自定义XML文档路径下的xml文件 Add By PGCAO
		$customXmlPath = self::path('extend/vqmod/xml', true);
		if(is_dir($customXmlPath)){
			self::$_modFileList = glob($customXmlPath . DIRECTORY_SEPARATOR . '*.xml');
		}
		// 固定XML文档路径为当前文件同根下的xml目录 Add By PGCAO
		$xmlPath = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'xml' . DIRECTORY_SEPARATOR;
		self::$_modFileList = array_unique(array_merge(glob($xmlPath . '*.xml'), self::$_modFileList));
		...
		$xml_folder_time = filemtime($xmlPath); // Edit By PGcao
		...
		if(self::$_devMode || !file_exists($modCache)) {
			...
			if(!empty($mods))
				self::$_mods = substr($mods, 0, 2) == 'a:' ? unserialize($mods) : false; // Edit By PGcao
			if(self::$_mods !== false) {
				return;
			}
		}
	...
	private static function _parseMods() {
		...
		foreach(self::$_modFileList as $modFileKey => $modFile) {
			if(file_exists($modFile)) {
				try {
					$dom->load($modFile);
					/** @var DOMDocument */ //Add By PGcao
					$mod = $dom->getElementsByTagName('modification')->item(0);
					/** @var DOMElement */ //Add By PGcao
			...
			if(count($modParts) !== count($checkParts)) {
				...
				foreach($toCheck as $k => $part) {
					if($part === '*') {
						...
						//$part = preg_replace_callback('~([^*]+)~', array('self', '_quotePath'), $part); // Remove By PGcao
...
class VQModLog {
	...
	public function __destruct() {
		...
		### 生成不存在的日志目录 Add By PGCAO
		$logPath = VQMod::path(VQMod::$logFolder, true);
		if (!is_dir($logPath))
			@mkdir($logPath);
		$logPath = $logPath . date('w_D') . '.log';

		$txt = array();

		$txt[] = str_repeat('-', 10) . ' Date: ' . date('Y-m-d H:i:s') . ' ~ IP : ' . (isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 'N/A') . ' ' . str_repeat('-', 10);
		$txt[] = 'REQUEST URI : ' . (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : ''); // Edit By PGcao
...
class VQModObject {
	...
	public function __construct(DOMNode $node, $modFile) {
		...
		### 去掉xml文档绝对路径 Add By PGCAO
		$modFile = str_replace(VQMod::getCwd(), '', $modFile);
	...
	public function applyMod($mods, &$data) {
		...
		foreach($mods as $mod) {
			...
			$tmp = (array) $this->_explodeData($tmp); // Edit By PGcao
			$lineMax = count($tmp) - 1;
	 ...
	 *
	 * @param DOMDocument $node <modification> node to be parsed Edit By PGcao
	 * @return null
		...
		foreach($files as $file) {
			...
			foreach($filesToMod as $filename) {
				...
				foreach($operations as $opIndex => $operation) {
					...
					### 去掉要修改的PHP文档绝对路径 Add By PGCAO
					$fullPath = str_replace(VQMod::getCwd(), '', $fullPath);
					if(!$skipOperation) {
